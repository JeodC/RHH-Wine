...
GAMEDIR="/$directory/windows/tearscape"
EXEC="$GAMEDIR/data/Tearscape.exe"
BASE=$(basename "$EXEC")
...
# Splash
chmod 777 "$SPLASH"
"$SPLASH" "$GAMEDIR/splash.png" 20000 &
...
declare -A DEP_DLL_MAP=(
    ...
    [dotnetdesktop8]="drive_c/Program Files/dotnet/dotnet.exe"
)
...
# Run the game
$GPTOKEYB "$BASE" -c "$GAMEDIR/tearscape.gptk" &
$BOX $WINE "$EXEC"
